...
prefix = '.';
name = 'Fbot';
rdaftar = 'OBRIGADO POR SE REGISTAR PARA SE TORNAR UM amigo do Pires 😁';
rmenu = 'Olá Pires AMIGOS 👋🏻 NÃO SE ESQUEÇA DE DOAR YAA :) ';
botinfo = 'PARA OS BOTS CONVIDADOS, DOE PRIMEIRO SIM :) ';
limitt = 999999999;
memberLimit = 2;
ban = [];
premium = ['556996103915@s.whatsapp.net', '556996103915@s.whatsapp.net'];
   ...
   fahmi.on('group-participants-update', async (anu) => {
      ...
      try {
         ...
         if (anu.action == 'add') {
            ...
            teks = `*_Oi👋_* @${num.split('@')[0]}\n ʙᴇᴍ ᴠɪɴᴅᴏ ᴀ ᴇsᴛᴇ ɢʀᴜᴘᴏ, ᴍᴇɴᴏs ɴᴏᴛᴀ ғᴀᴋᴇ ᴇ ᴢᴇ ᴛʀᴀᴠɪɴʜᴀ *${mdata.subject}*\n\n ────────────────
┏━━━━━━━━━━━━━━━
┃────「 *_ᴀᴘʀᴇsᴇɴᴛᴀÇÃᴏ_* 」─────
┃━━━━━━━━━━━━━━━
┠⊷️ *ɴᴏᴍᴇ* :
...
┗━━━━━━━━━━━━━━━`;
            ...
            teks = `*_「 rebaixado detetado 」_*\n@${num.split('@')[0]} quem é um administrador quem?, já está em um grupo ainda! `;
            let buff = await getBuffer(ppimg);
            fahmi.sendMessage(mdata.id, buff, MessageType.image, { caption: teks, contextInfo: { mentionedJid: [num] } });
            ...
            teks = `*_Adeus👋_* @${num.split('@')[0]} sᴇ ᴠᴏᴄᴇ sᴀɪᴜ ᴘᴏʀ ᴀʟɢᴜᴍ ɪɴᴄᴏᴠᴇɴɪᴇɴᴛᴇ ᴅᴇsᴄᴜʟᴘᴇ, ᴍᴀs sᴇ ᴄᴀᴜsᴏᴜ ᴀʟɢᴜᴍ ᴘʀᴏʙʟᴇᴍᴀ ᴏᴜ ғᴏʀ ɴᴏᴛᴀs ғᴀᴋᴇs ɴᴀᴏ ᴘʀᴇᴄɪsᴀ ᴠᴏʟᴛᴀʀ`;
            let buff = await getBuffer(ppimg);
            fahmi.sendMessage(mdata.id, buff, MessageType.image, { caption: teks, contextInfo: { mentionedJid: [num] } });
         }
      } catch (e) {
   ...
   fahmi.on('chat-update', async (mek) => {
      try {
         ...
         mess = {
            wait: '*⏳ ⏳ ᴇsᴘᴇʀᴇ ᴛᴏᴍᴇ  ᴜᴍa água ᴇɴǫᴜᴀɴᴛᴏ ᴠᴏᴜ fazer...*',
            success: '*Sucesso camarada...*',
            error: {
               stick: '*Falha, ocorreu um erro ao conectar a imagem ao adesivo*',
               Iv: '*Desculpe, o link não é válido!*',
            },
            only: {
               group: '*Desculpe, este comando só pode ser usado em grupos!*',
               benned: '*Desculpe, seu número foi banido, por favor entre em contato com Oᴡne para abrir seu banimento*',
               ownerG: '*Desculpe, este comando só pode ser usado por um grupo único!*',
               ownerB: '* Desculpe, este comando só pode ser usado por um bot!* ',
               premium: '*Desculpe, este recurso é apenas para uso premium! * ',
               userB: `*Oi como ${pushname2} você não está registrado no banco de dados, digite* \n${prefix}daftar`,
         ...
         const ownerNumber = ['556996103915@s.whatsapp.net', '556996103915@s.whataapp.net']; // owner number ubah aja
         ...
         if (messagesLink.includes('556996103915')) {
            // respon tag ubah aja
         ...
         switch (command) {
            case 'menu':
               ...
               const Menu = {
                  text: `      「 *_Informação de usuário_* 」

» Nome: ${pushname2}
» Limite: ${limitt}
» Prefix: ${prefix}
» Registrado: √
» Registro: ${user.length} 
...
Juntar-se ao grupo :
https://chat.whatsapp.com/CWDE10hFK31JwBN79IWJLB
...
Create By _@fahmicog_`,
            ...
            case 'antilink':
               ...
               if (!isGroupAdmins) return reply(mess.only.admin);
               if (!isBotGroupAdmins) return reply(mess.only.Badmin);
               if (args.length < 1) return reply('digite #antilink ativado para ativar');
               if (args[0] === 'on') {
                  if (isAntiLink) return reply('anti-link está ativado');
                  antilink.push(from);
                  ...
                  reply('ligado para habilitar, desligado para desabilitar');
               }
               break;
            case 'antibadword':
               if (!isGroup) return reply(mess.only.group);
               if (!isGroupAdmins) return reply(mess.only.admin);
               if (args.length < 1) return reply('ligado para habilitar, desligado para desabilitar ');
               if (args[0] === 'on') {
                  if (isBadWord) return reply('anti badword está ativado');
                  badword.push(from);
                  fs.writeFileSync('./database/json/badword.json', JSON.stringify(badword));
                  reply(`\`\`\`✓“Ativar com sucesso o recurso anti-badword no grupo\`\`\` *${groupMetadata.subject}*`);
               } else if (args[0] === 'off') {
                  if (!isBadWord) return reply('anti badword está desligado ');
            ...
            case 'delbadword':
               if (!isOwner) return reply(mess.only.ownerB);
               if (args.length < 1) return reply(`Enviar pedidos ${prefix}delbadword [kata kasar]. exemplo ${prefix}delbadword estúpido`);
            ...
            case 'jadwaltv':
               ...
               if (!isUser) return reply(mess.only.userB);
               ch = body.slice(10);
               if (args.length < 1) return reply('Digite o nome do canal ');
               fahmi.updatePresence(from, Presence.composing);
               reply(mess.wait);
               try {
            ...
            case 'tomp3':
               ...
               exec(`ffmpeg -i ${media} ${ran}`, (err) => {
                  fs.unlinkSync(media);
                  if (err) return reply('Falha ao converter o vídeo ke mp3');
            ...
            default:
               ...
               if (isGroup && isSimi && budy != undefined) {
                  ...
                  console.log(color('[Fbot', 'aqua'), 'Comando não registrado ', cor(sender.split('@')[0]));
               }
         }
      } catch (e) {
